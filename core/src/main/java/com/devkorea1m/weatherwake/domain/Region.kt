...
enum class Region {
    ...
    companion object {
        ...
        fun fromCoordinates(lat: Double, lon: Double): Region = when {
            ...
            //
            // 북쪽 경계 38.7°N 의도: **남한 최북단 지역(고성·철원·백령도 등)** 만
            // 포함하고 북한 영역(평양 39.03°N 이상) 은 의도적으로 제외.
            // 대한민국 헌법상 북한도 KR 영토이나 KMA 의 ASOS/AWS 관측소가
            // 남한에만 있어 북한 좌표 질의 시 -999 sentinel 이 반환된다. PR #25
            // 이후엔 sentinel → OWM 폴백이 자동 작동하므로 기능적 결과는
            // 동일하지만, 불필요한 KMA 네트워크 왕복을 피하고 OTHER → OWM
            // 단독 경로로 바로 가도록 제외한 것. UI attribution 은 양쪽 모두
            // "OpenWeatherMap 제공" 으로 동일하게 표시됨.
